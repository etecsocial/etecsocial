@foreach(App\ComentarioDiscussao::where('discussao_id', $discussao_id)->where('id', '>', $comentario_id)->get() as $comentario)
<li id="com-disc-{{ $comentario->id }}" class="collection-item avatar com-disc-{{ $discussao_id }}" style="height: auto; min-height:65px;max-height: 100%" data-id="{{ $comentario->id }}">
